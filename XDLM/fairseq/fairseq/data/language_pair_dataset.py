...
def collate(
    ...
    batch = {
        ...
        "net_input": {
            ...
            "src_language_emb": src_language_emb,
            "src_position_emb": src_position_emb 
        },
        "target": target,
    }
...
class LanguagePairDataset(FairseqDataset):
    ...
    def __init__(
        ...
        self.pad_index=src_dict.pad()
        self.eos_index=src_dict.eos()
        self.unk_index=src_dict.unk()
        self.mask_index=src_dict.mask()
        self.n_words=len(src_dict)
        self.word_pred=word_pred
        self.sample_alpha=sample_alpha
        self.pred_probs=torch.FloatTensor(word_mask_keep)
        if tgt is not None:
            assert len(src) == len(
                tgt
        ...
        # logger.info(self.src_sizes.shape)--> (n,)
        # logger.info(num_buckets)--> 0
        if num_buckets > 0:
