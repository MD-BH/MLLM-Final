...
def _get_attention_head_zero_patch_mode(attention_type: str) -> str:
    return f"{attention_type}_attn_head_zero_ablation"


def _run_decoder_attention_head_zero_forward(
    model,
    tgt_tokens: torch.Tensor,
    encoder_out: Dict[str, torch.Tensor],
    layer_index: int,
...
def _build_head_zero_patch_trace_step(
    *,
    attention_type: str,
    layer_index: int,
    head_index: int,
    ...
    return {
        "iteration": int(iteration),
        "patch_mode": _get_attention_head_zero_patch_mode(attention_type),
        "module_name": _get_attention_module_name(attention_type, layer_index),
...
def _decoder_attention_head_zero_ablation(
    attention_type: str,
    target_sentence: Optional[str] = None,
    context: Optional[Dict[str, object]] = None,
    layer_index: Optional[int] = None,
    ...
    with torch.no_grad():
        decoder_out, capture = _run_decoder_attention_head_zero_forward(
            model,
            target_tgt_tokens,
            target_encoder_out,
            layer_index,
        ...
        _record_iteration(iteration_trace, 0, target_tgt_tokens, target_token_probs, tgt_dict, args.remove_bpe)
        patch_trace.append(
            _build_head_zero_patch_trace_step(
                attention_type=attention_type,
                layer_index=layer_index,
                head_index=head_index,
                capture=capture,
...
def _decoder_attention_head_zero_ablation_sweep(
    attention_type: str,
    target_sentence: Optional[str] = None,
    context: Optional[Dict[str, object]] = None,
    layer_index: Optional[int] = None,
    ...
    for head_index in head_indices:
        head_result = _decoder_attention_head_zero_ablation(
            attention_type=attention_type,
            target_sentence=target_sentence,
            context=context,
            layer_index=layer_index,
...
def decoder_self_attn_head_zero_ablation(
    ...
    return _decoder_attention_head_zero_ablation(
        "self",
        target_sentence=target_sentence,
        context=context,
        layer_index=layer_index,
...
def decoder_self_attn_head_zero_ablation_sweep(
    ...
    return _decoder_attention_head_zero_ablation_sweep(
        "self",
        target_sentence=target_sentence,
        context=context,
        layer_index=layer_index,
