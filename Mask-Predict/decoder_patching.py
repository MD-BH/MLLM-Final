...
def decoder_attention_zero_out(
    attention_type: str,
    target_sentence: Optional[str] = None,
    context: Optional[Dict[str, object]] = None,
    layer_index: Optional[int] = None,
...
def decoder_attention_zero_out_sweep(
    attention_type: str,
    target_sentence: Optional[str] = None,
    context: Optional[Dict[str, object]] = None,
    layer_index: Optional[int] = None,
    ...
    for head_index in head_indices:
        head_result = decoder_attention_zero_out(
            attention_type=attention_type,
            target_sentence=target_sentence,
            context=context,
            layer_index=layer_index,
...
def _normalize_selected_layer_indices(
    layer_indices: List[int],
    selected_layer_indices: Optional[List[int]],
    *,
    name: str,
) -> List[int]:
    ...
    normalized_selected_indices = [int(layer_index) for layer_index in selected_layer_indices]
    layer_index_set = set(layer_indices)
    invalid_layer_indices = [
        layer_index
        for layer_index in normalized_selected_indices
        if layer_index not in layer_index_set
    ]
    if invalid_layer_indices:
        raise IndexError(
            f"{name} contains layers not present in layer_indices: {invalid_layer_indices}"
        )
    return normalized_selected_indices


def decoder_attention_zero_out_layer_sweep(
    attention_type: str,
    target_sentence: Optional[str] = None,
    context: Optional[Dict[str, object]] = None,
    decoding_iterations: int = 5,
    layer_indices: Optional[List[int]] = None,
    head_indices: Optional[List[int]] = None,
    average_layer_indices: Optional[List[int]] = None,
    tgt_sentence: Optional[str] = None,
) -> Dict[str, object]:
    if context is None:
        raise ValueError("context is required")
    ...
    decoder_layers = model.decoder.layers
    layer_indices = _normalize_layer_indices(decoder_layers, layer_indices)
    average_layer_indices = _normalize_selected_layer_indices(
        layer_indices,
        average_layer_indices,
        name="average_layer_indices",
    )
    average_layer_index_set = set(average_layer_indices)

    layer_results: List[Dict[str, object]] = []

    for layer_index in layer_indices:
        layer_result = decoder_attention_zero_out_sweep(
            attention_type=attention_type,
            target_sentence=target_sentence,
            context=context,
            layer_index=layer_index,
    ...
    averaged_layer_results = [
        layer_result
        for layer_result in layer_results
        if layer_result["layer_index"] in average_layer_index_set
    ]
    if not averaged_layer_results:
        raise ValueError("average_layer_indices resolved to an empty set")

    average_heatmap = (
        torch.tensor(
            [layer_result["heatmap"] for layer_result in averaged_layer_results],
            dtype=torch.float32,
        )
        .mean(dim=0)
        .tolist()
    ...
    return {
        ...
        "patch_mode": _get_attention_zero_out_patch_mode(attention_type),
        "layer_indices": layer_indices,
        "head_indices": layer_results[0]["head_indices"] if layer_results else [],
        "iterations": layer_results[0]["iterations"] if layer_results else [],
        "reference_decoded_text": layer_results[0]["reference_decoded_text"] if layer_results else "",
        "reference_token_ids": layer_results[0]["reference_token_ids"] if layer_results else [],
        "average_layer_indices": average_layer_indices,
...
def decoder_self_attn_zero_out(
    ...
    return decoder_attention_zero_out(
        "self",
        target_sentence=target_sentence,
        context=context,
        layer_index=layer_index,
...
def decoder_self_attn_zero_out_sweep(
    ...
    return decoder_attention_zero_out_sweep(
        "self",
        target_sentence=target_sentence,
        context=context,
        layer_index=layer_index,
...
def decoder_self_attn_zero_out_layer_sweep(
    target_sentence: Optional[str] = None,
    context: Optional[Dict[str, object]] = None,
    decoding_iterations: int = 5,
    layer_indices: Optional[List[int]] = None,
    ...
    return decoder_attention_zero_out_layer_sweep(
        "self",
        target_sentence=target_sentence,
        context=context,
        decoding_iterations=decoding_iterations,
...
def decoder_cross_attn_zero_out_layer_sweep(
    target_sentence: Optional[str] = None,
    context: Optional[Dict[str, object]] = None,
    decoding_iterations: int = 5,
    layer_indices: Optional[List[int]] = None,
